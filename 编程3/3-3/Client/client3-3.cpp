...
//宏定义滑动窗口大小、超时参数、Reno算法相关参数
//=================================================================================
//Reno拥塞控制算法实现
//全局变量拥塞窗口大小，根据Reno算法，初始值为1
int windowlength = 1;
//全局变量慢开始门限，这里初始值设为16
int ssthresh = 16;
#define timelimit 100 //100ms
#define timelimit_udp 300 //窗口内的报文0.15s未确认则重传
...
//初始化数据报组接收确认数组
int acked[65536];//记录ACK次数的数组，便于进行识别快重传
bool isthread[65536];
...
void print_red_message(int seq) {
    ...
    // 打印带颜色的输出
    cout << RED << "序列为" << seq << "的报文超时未确认，触发重传机制,并调整窗口大小与慢启动门限阈值." << RESET << endl;
}
...
//快重传函数
int quickresend(int tri_ack) {
    int seq = tri_ack + 1;//ack重复三次的变量说明下一个数据报丢包了
...
void Reno() {
    if (!RenoMode) // 慢启动
    {
        std::cout << GREEN << "Mode: 慢启动\n" << RESET;
        if (2 * windowlength > ssthresh) // 达到阈值，进入拥塞避免状态
        {
            RenoMode = 1; // 调整模式标志位
            windowlength = ssthresh;
            std::cout << GREEN << "Threshold reached, 切换到拥塞避免模式.\n" << RESET;
        }
        else {
            windowlength *= 2;
        }
    ...
    {
        std::cout << GREEN << "Mode: 拥塞避免\n" << RESET;
        windowlength += 1;
    }

    // 打印当前窗口大小
    std::cout << YELLOW << "当前窗口大小: " << windowlength << "\n" << RESET;
    std::cout << YELLOW << "当前门限大小:" << ssthresh << "\n" << RESET;
}
...
// 注意差错的报文也是未确认，启动超时检测重传机制
    ...
    while (true)
    {
        ...
        {
            //向上取整
            ssthresh = (windowlength + 1) / 2 ; // 向上取整除以 2
            windowlength = ssthresh;
...
int receive() {
    while (true)
    {
        ...
        else if (recv.getstate() & ACK && !(recv.getstate()&SYN))
        {
            int ack = recv.getack();
            if (ack == 65535)
                ack = -1;
            if(accumulate_point!= ack)
            {
                
                //cout << ack << endl;
                //cout << accumulate_point;
                for (int j = accumulate_point + 1; j <= ack; j++)
                    acked[j] += 1;          // 更新状态，表示累积确认
                //cout << "累积确认，序列号" << recv.getack() << "之前的所有数据报已确认！" << endl;
                accumulate_point = ack;//更新累积确认指针
                //cout << "===============================================================" << endl;
            }
            else
            {
                cout << "重复ACK!!!" << ack << "的序列号报文重复ACK！" << endl;
                acked[accumulate_point] += 1;
                cout << "===============================================================" << endl;
            }
        ...
        //===================================================================
        //检测快重传函数：关键
        if (acked[accumulate_point] == 4)//所谓事不过三，过三速速重传！
        {
            std::cout << GREEN << "三次重复ack, 进行快重传，并调整窗口大小与慢启动门限阈值.\n" << RESET;
            //快重传该报文
            quickresend(accumulate_point + 1);
            //调整窗口大小和慢启动门限
            //向上取整
            ssthresh = (windowlength + 1) / 2 + 3; // 向上取整除以 2再加三
            windowlength = ssthresh;
...
void windowsending() {
    while (true) {
        ...
        if (acked[windowtop])
        {
            ...
            //滑动窗口进入下一个分组 
            windowbase=windowtop+1;        
            //根据windowlength来调整窗顶指针
            windowtop = windowbase + windowlength-1;
...
int main() {
    ...
    string path;
    //cin >> path;
    path = "D:/28301/Desktop/new/2.jpg";
